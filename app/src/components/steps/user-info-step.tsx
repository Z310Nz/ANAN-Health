...
export default function UserInfoStep({ onNext, onClear }: UserInfoStepProps) {
  ...
  const calculateAndSetCoverageUntilAge = useCallback(() => {
    const currentAge = getValues('userAge');
    const period = getValues('coveragePeriod');
    if (typeof currentAge === 'number' && typeof period === 'number' && period > 0) {
  ...
  useEffect(() => {
    calculateAndSetCoverageUntilAge();
  }, [userAge, coveragePeriod, calculateAndSetCoverageUntilAge]);
  ...
  const handleCoverageUntilAgeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(coverageUntilAge) && typeof currentAge === 'number' && coverageUntilAge > currentAge) {
      const newCoveragePeriod = coverageUntilAge - currentAge;
      // Max coverage until age 100
      if (coverageUntilAge <= 100) {
        setValue('coveragePeriod', newCoveragePeriod, { shouldValidate: true });
      } else {
        setValue('coveragePeriod', 100 - currentAge, { shouldValidate: true });
        setCoverageUntilAgeDisplay('100');
      }
    }
  };
      ...
      <CardContent className="space-y-6 max-w-sm mx-auto">
        ...
        <FormItem>
            ...
            <FormControl>
              <Input 
                type="number" 
                placeholder="สูงสุด 100 ปี" 
                onChange={handleCoverageUntilAgeChange}
                value={coverageUntilAgeDisplay}
              />
            </FormControl>
            {errors.coveragePeriod && <FormMessage>{errors.coveragePeriod.message}</FormMessage>}
        </FormItem>
      </CardContent>
